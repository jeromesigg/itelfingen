<section id="gallery_section" class="gallery section">
  <div class="container" >
    ...
    <div id="indicators-carousel" class="relative w-full" data-carousel="static">
      <!-- Carousel wrapper -->
      <div class="relative h-56picture overflow-hidden rounded-lg md:h-picture">
          <!-- Item 1 -->
          @foreach ($pictures as $key=>$picture)
          <div class="hidden duration-700 ease-in-out" data-carousel-item="{{$key==0 ? 'active':''}}">
              <img src="{{$picture->photo->file}}" class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="{{$picture->name}}">
          </div>   
          @endforeach    
      </div>
      <!-- Slider indicators -->
